...
public class LocalStorage extends Storage {
     ...
     */
    @Override public void setList(SyncedList list)
            throws IOException, JSONException {
        SharedPreferences.Editor editor = sharedPref.edit();
        editor.putString("LIST_"+list.getId(), list.toJSON().toString());
        editor.apply();
        Log.d(Constant.LOG_TITLE_DEFAULT,
              "Save List: " + list.getName());
    }

    /**
     * Read on list from local storage
     * 
     * @param syncedListHeader
     * @return SyncedList
     * @throws IOException
     * @throws JSONException
     */
    @Override public SyncedList getList(SyncedListHeader syncedListHeader)
            throws IOException, JSONException {
        String data = sharedPref.getString("LIST_"+syncedListHeader.getId(),
                                           "");
        if (data.equals("")) {
            return null;
        }
        SyncedList result = new SyncedList(syncedListHeader,
                                           new JSONObject(data));
        Log.d(Constant.LOG_TITLE_DEFAULT,
    ...
    @Override public ArrayList<SyncedListHeader> getListsHeaders()
        ...
        for(int i=0; i<jsonArrayLists.length(); i++) {
            result.add(new SyncedListHeader((JSONObject) jsonArrayLists.get(i)));
        }
        Log.d(Constant.LOG_TITLE_DEFAULT,
              "Load Lists Headers: " + result.toString());
        return result;
